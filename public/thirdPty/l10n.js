...
(function () {
	...
	, request_JSON = function (uri) {
		...
		if (req.status !== 200) {
		    ...
		    return JSON.parse(req.responseText.split("/*#*/")[1]);
		}
	}
	...
	if (typeof document !== undef_type) {
		...
		while (i--) {
			var
                  ...
                  rel = [(/Localization/i.test(elt.src)?"localization":"")]
			;
			// Changes by SasaRev for it to work with CDE_JS_Solutions
			//if (l10n_js_media_type.test(elt.type)) {
				if (array_index_of.call(rel, "localizations") !== -1) {
				    ...
				    var file = elt.src.split('/')[elt.src.split('/').length - 1];
				    var lang = file.split('.')[0].split('_')[1];

				    localization[(lang || "")] = elt.getAttribute("src");;
					load(localization);
				}
			//}
		}
